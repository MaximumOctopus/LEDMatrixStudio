					...
					else
					{
						if (l == CurrentLayer)
						{
							if (LightBox == 1 && CurrentFrame != 0)
							{
								...
								{
									MatrixRender->Grid[y * Details.Width + x] = ColourUtility::BrightenRGB(MatrixLayers[l]->Cells[CurrentFrame - 1]->Grid[y * Details.Width + x]);
								}
							}
						...
						if (l == CurrentLayer)
						{
							if (LightBox == 1 && CurrentFrame != 0)
							{
								if (DisplayBuffer->Grid[y * Details.Width + x] == RGBBackground)
								{
									if (DisplayBuffer->Grid[y * Details.Width + x] != RGBBackground)
									{
										MatrixRender->Grid[y * Details.Width + x] = ColourUtility::BrightenRGB(LEDRGB3BPPColours[MatrixLayers[l]->Cells[CurrentFrame - 1]->Grid[y * Details.Width + x]]);
									}
								}
							}
							else
...
void __fastcall TheMatrix::ClickPixel(TObject *Sender, TMouseButton Button, TShiftState Shift, int X, int Y)
{
	...
	if (Shift.Contains(ssLeft))
	{
		...
		case DrawMode::kPaste:
			DrawWithBrushPaste(x1, y1, false);//!(Shift.Contains(ssLeft))); // dont need this contains?!
...
void __fastcall TheMatrix::ClickPixelBiColour(TObject *Sender, TMouseButton Button, TShiftState Shift, int X, int Y)
{
	...
	if (Shift.Contains(ssLeft))
	{
		...
		case DrawMode::kMulti:
			...
			break;
		case DrawMode::kPaste:
			DrawWithBrushPaste(x1, y1, false);
...
void TheMatrix::ClearGradients()
{
	for (int t = 0; t < Details.Width; t++)
	{
		if (Details.Mode == MatrixMode::kRGB)
		{
			Render.Gradient.IX[t] = RGBBackground;
		}
		else
		{
			Render.Gradient.IX[t] = 0;
		}
	}

	for (int t = 0; t < Details.Height; t++)
	{
		if (Details.Mode == MatrixMode::kRGB)
		{
			Render.Gradient.IY[t] = RGBBackground;
		}
		else
		{
			Render.Gradient.IY[t] = 0;
		}
	}
}
...
void TheMatrix::InsertBlankFrameAt(int insertat)
{
	...
	for (int layer = 0; layer < MatrixLayers.size(); layer++)
	{
		...
		{
			MatrixLayers[layer]->Cells.insert(MatrixLayers[layer]->Cells.begin() + insertat + 1, m);
		}
	}
...
void TheMatrix::InsertCopyFrameAt(int insertat)
{
	...
	for (int layer = 0; layer < MatrixLayers.size(); layer++)
	{
		...
		if (insertat == MatrixLayers[layer]->Cells.size() - 1)
		{
			MatrixLayers[layer]->Cells.push_back(m);
		}
		else
		...
		for (int frame = 0; frame < GetFrameCount(); frame++)
		{
			...
			for (int y = 0; y < Details.Height; y++)
			{
				...
				for (int x = 0; x < Details.Width; x++)
				{
					...
					ptr[((frame - 1) * Details.Width) + x].rgbtRed = (colour & 0x0000ff);
					ptr[((frame - 1) * Details.Width) + x].rgbtGreen = (colour & 0x00ff00) >> 8;
					ptr[((frame - 1) * Details.Width) + x].rgbtBlue = (colour & 0xff0000) >> 16;
				}
			}
		}
...
bool TheMatrix::ExportAnimationToBitmap(const std::wstring file_name)
{
	...
	try
	{
		...
		for (int frame = 0; frame < GetFrameCount(); frame++)
		{
            ...
            std::wstring output = prefix + L"_" + Formatting::PadZeroes(std::to_wstring(frame), 6) + L".bmp";
...
void TheMatrix::FadeFirstToLast()
{
	for (int y = 0; y < Details.Height; y++)
	{
		for (int x = 0; x < Details.Width; x++)
		{
			...
			for (int frame = 2; frame < GetFrameCount(); frame++)
			{
				newr  = newr + rdx;
				newg  = newg + gdx;
				newb  = newb + bdx;
...
/*
int TheMatrix::GetColourFromXY(int x, int y)
{
	switch (Details.Mode)
	{
	case MatrixMode::kRGB:
		return DisplayBuffer->Grid[y * Details.Width + x];
	case MatrixMode::kRGB3BPP:
		return LEDRGB3BPPColours[DisplayBuffer->Grid[y * Details.Width + x]];
